...
def train_gated_mdn(
    ...
    ----------
    X_train : array (N, 3)
        Training inputs (already normalised): (log10(N_tot), alpha, beta).
    Y_train : array (N,)
    ...
    X_val, Y_val, resolved_flag_val : optional arrays
        Validation data.
    n_components, n_hidden, n_steps, lr, batch_size, print_every, patience :
        Hyperparameters.  ``patience`` is the number of steps without
        validation loss improvement before early stopping (requires val data).
    ...
    for i in range(n_steps):
        ...
        # Compute validation loss every step for early stopping
        ...
        if X_val is not None and patience and steps_without_improvement >= patience:
            print(f"Early stopping at step {i}: no val improvement for {patience} steps. "
                  f"Best val loss = {best_val_loss:.4f}")
            break

    # Return best params (by validation loss) if we have val data
    if X_val is not None:
        params = best_params
