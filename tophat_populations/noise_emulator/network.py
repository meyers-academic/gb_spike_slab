"""
Mixture Density Network for confusion noise emulation (single band).

Maps population hyperparameters (log10(N_tot), alpha, beta) to a conditional
density over log S_conf using a 1D Gaussian mixture model with a gate for the
fully-resolved case (S_conf = 0).

The MDN only models p(log S_conf | Λ). The resolved source count N_res is
...
def init_gated_mdn_params(key, n_hidden=64, n_components=5):
    """
    Initialise parameters for the gated MDN (single-band).

    Input dimension is 3: (log10(N_tot), alpha, beta).
    ...
    params = {
        'layer1': init_layer(k1, 3, n_hidden),
...
def gated_mdn_forward(params, x, n_components=5):
    ...
    x : array (..., 3)
        Input features (log10(N_tot), alpha, beta).
...
def compute_lambda_res(log_S_conf, N_tot, alpha, beta, f_k,
    ...
    """
    Compute the Poisson rate lambda_res for resolved sources in a single band.
    ...
    f_k : float
        Band center frequency (fixed config parameter).
    S_instr_k : float
        Instrumental noise PSD in the band.
    T_obs : float
        Observation time [s].
    rho_th : float
